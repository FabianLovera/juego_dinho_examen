...
class Obstacle:
    ...
    def update(self):
        if self.image == bird_img:
            self.rect.y -= self.speed  # Mover solo hacia arriba
        else:
            self.rect.x -= self.speed  # Movimiento horizontal por defecto

        if self.rect.x < -self.rect.width or self.rect.y < -self.rect.height:
            self.rect.x = WIDTH + random.randint(300, 600)
...
def display_scores(scores):
    ...
    for i, score in enumerate(scores[:5]):
        ...
        screen.blit(
            score_text, (WIDTH // 2 - score_text.get_width() // 2, 100 + i * 40)
        )
...
def show_start_screen():
    ...
    start_text = font.render("Presiona ESPACIO para empezar", True, YELLOW_RED)
    ...
    while waiting:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                waiting = False
    pygame.mixer.music.fadeout(500)  # Detener música de fondo
        ...
        if not paused:
            ...
            if not obstacles:
                for obstacle in obstacles_to_add:
                    if isinstance(obstacle, FallingMeteor):
                        obstacles.append(FallingMeteor(obstacle.image, obstacle.speed))
                    else:
                        obstacles.append(Obstacle(obstacle.image, obstacle.speed))
            ...
            for obstacle in obstacles:
                if isinstance(obstacle, FallingMeteor):
                    obstacle.update(dinosaur.rect)
                else:
                    obstacle.update()

            # Comprobar colisiones
            for obstacle in obstacles:
                if dinosaur.rect.colliderect(obstacle.rect.inflate(-10, -10)):
                    game_over = True
                    break
            ...
            if score % 500 == 0 and score != 0:
                ...
                if (
                    ...
                    if isinstance(new_obstacle, FallingMeteor):
                        obstacles.append(
                            FallingMeteor(new_obstacle.image, new_obstacle.speed)
                        )
                    else:
                        obstacles.append(
                            Obstacle(new_obstacle.image, new_obstacle.speed)
                        )
